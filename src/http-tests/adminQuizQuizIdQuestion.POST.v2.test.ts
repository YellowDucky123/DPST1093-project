...
describe('POST /v2/admin/quiz/:quizId/question', () => {
  ...
  test('error invalid token test', () => {
    // test about no body
    let res = request('POST', root + `/v2/admin/quiz/${quizId}/question`);
    expect(res.statusCode).toBe(TOKEN_ERROR);
    // test about not exist token
    res = request('POST', root + `/v2/admin/quiz/${quizId}/question`, { json: details, headers: { token: details.token } });
    expect(res.statusCode).toBe(TOKEN_ERROR);
  });
  test('error of valid token is provided, but user is not an owner of this quiz', () => {
    // regist user1
    let res = request('POST', root + '/v2/admin/auth/register', { json: userDetails1, headers : {token : userDetails1.token} });
    expect(res.statusCode).toBe(OK);
    userDetails1.token = JSON.parse(res.body as string).token;

    // regist user2
    res = request('POST', root + '/v2/admin/auth/register', { json: userDetails2, headers : {token : userDetails2.token} });
    expect(res.statusCode).toBe(OK);
    userDetails2.token = JSON.parse(res.body as string).token;

    // create a quiz for user1
    res = request('POST', root + '/v2/admin/quiz', { json: { token: userDetails1.token, name: 'test quiz', description: 'quiz of user1' }, headers : {token : userDetails1.token} });
  ...
  test('create successful for user1', () => {
    ...
    const res = request('POST', root + `/v2/admin/quiz/${quizId}/question`, { json: details, headers : {token : details.token} });
    expect(res.statusCode).toBe(OK);
  ...
  test('error Question string is less than 5 characters in length or greater than 50 characters in length', () => {
    ...
    let res = request('POST', root + `/v2/admin/quiz/${quizId}/question`, { json: details, headers : {token : details.token} });
